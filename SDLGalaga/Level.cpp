...
Level::Level(int stage, PlaySideBar* sideBar, Player* player)
{
    ...
    //init player in scene and handle respawn variables
    mPlayer = player;
    mPlayerHit = false;
    ...
    //load xml file for challenge stages
    ...
    //init varialbes for choosing diving butterfly
    mDivingButterfly = NULL;
    mSkipFirstButterfly = false;
    mButterflyDiveDelay = 1.0f;
    mButterflyDiveTimer = 0.0f;

    //init varialbes for choosing diving wasp
    mDivingWasp = NULL;
    mDivingWasp2 = NULL;
    mWaspDiveDelay = 3.0f;
    mWaspDiveTimer = 0.0f;

    //init varialbes for choosing diving boss
    mDivingBoss = NULL;
    mCaptureDive = true;
    mSkipFirstBoss = true;
    mBossDiveDelay = 5.0f;
...
//stage reaction to player being hit
...
//respawn player and handle death
            ...
            for (int i = MAX_BOSSES - 1; i >= 0; --i)
            {
                if (mFormationBosses[i]->CurrentState() == Enemy::formation)
                {
                    if (!mSkipFirstBoss || (mSkipFirstBoss && skipped))
                    {
                        ...
                        {
                            //regular boss dive takes 2 butterflies with them
                            mDivingBoss->Dive();
                            int index = mDivingBoss->Index();
                            int firstEscortIndex = (index % 2 == 0) ? (index * 2) : (index * 2 - 1); //escort chosen with the help of index system talked about in report
